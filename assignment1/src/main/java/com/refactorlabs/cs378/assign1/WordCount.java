...
public class WordCount {
	...
	// Params: input key, input value, output key, output value
	public static class MapClass extends Mapper<LongWritable, Text, Text, LongWritable> {
		...
		// This word will be written out, this is a mutable text object
		private Text word = new Text();

		// takes the key and value
		// the context explains where data is from and where it's being written to, used by Main
		// context abstracts environmental details for where to write etc.
		@Override
		public void map(LongWritable key, Text value, Context context)
			...
			// a group name and counter name is passed in
			// counter name is used in the output
	...
	// input key, input value types are defined
	// output key, output value types are defined
	...
	public static void main(String[] args) throws Exception {
		...
		// Second argument as output path
		FileOutputFormat.setOutputPath(job, new Path(appArgs[1]));

		// Initiate the map-reduce job, and wait for completion.
		// This may be used to run multiple jobs to merge output from tasks
